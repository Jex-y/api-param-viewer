...
/// real newlines and markdown instead of JSON-escaped \n.
fn pretty_print_value(val: &Value, indent: usize, max_width: usize) -> Vec<Line<'static>> {
    ...
    match val {
        Value::Object(obj) => {
            let mut lines = Vec::new();
            lines.push(Line::from(Span::styled(format!("{}{{", indent_str), brace_style)));
            let entries: Vec<_> = obj.iter().collect();
            for (i, (k, v)) in entries.iter().enumerate() {
                let comma = if i + 1 < entries.len() { "," } else { "" };
                match v {
                    Value::String(s) if s.contains('\n') || s.len() > 80 => {
                        // Render key, then the string content as readable text below
                        lines.push(Line::from(vec![
                            Span::styled(format!("{}  ", indent_str), brace_style),
                            Span::styled(format!("\"{}\": ", k), key_style),
                        ]));
                        // Render the string as markdown/readable text, indented
                            ...
                            if text_width > 0 && text_line.len() > text_width {
                                ...
                                for word in words {
                                    if current.is_empty() {
                                        ...
                                        lines.push(Line::from(vec![
                                            Span::styled(format!("{}│ ", text_indent), Style::default().fg(Color::DarkGray)),
                ...
                match item {
                    ...
                    Value::String(s) => {
                        let display = if s.len() > 60 {
                            let clean: String = s.chars().take(57).map(|c| if c == '\n' { ' ' } else { c }).collect();
                            format!("\"{}...\"", clean)
                        } else {
                            format!("\"{}\"", s.replace('\n', "\\n"))
                        };
        ...
        Value::String(s) => {
            vec![Line::from(Span::styled(format!("{}\"{}\"", indent_str, s), str_style))]
        }
        Value::Number(n) => {
            vec![Line::from(Span::styled(format!("{}{}", indent_str, n), num_style))]
        }
        Value::Bool(b) => {
            vec![Line::from(Span::styled(format!("{}{}", indent_str, b), bool_style))]
        }
        Value::Null => {
            vec![Line::from(Span::styled(format!("{}null", indent_str), null_style))]
        }
    }
}
...
fn draw_detail(frame: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    let all_lines: Vec<Line<'static>> = match val {
        ...
        Some(val) => {
            pretty_print_value(val, 0, inner_width)
        }
        None => vec![],
    };
